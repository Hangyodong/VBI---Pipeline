...
def _load_optional_bold():
    if not config.BOLD_PATH or not os.path.exists(config.BOLD_PATH):
...
def _scale_weights(weights):
    """Apply compression + max-norm + sparse mask to SC.

    Compression mode via env ``VBI_SC_SCALE`` (default ``log1p``):
      ``log1p``   : log1p(w + 0.5) then max-norm  (original — heavy compression)
      ``maxnorm`` : raw counts then max-norm       (NO compression)
      ``sqrt``    : sqrt(w) then max-norm           (mild compression)
    The compression squeezes the count dynamic range; ``maxnorm`` keeps the
    full range to test whether log1p over-homogenizes coupling (FC misspec
    diagnosis, see docs/03).
    """
    mode = os.environ.get("VBI_SC_SCALE", "log1p").lower()
    ...
    elif mode in ("maxnorm", "raw", "none"):
        pass  # no compression — raw counts straight to max-norm
    else:
        raise ValueError(f"unknown VBI_SC_SCALE={mode!r} "
                         f"(use log1p|maxnorm|sqrt)")

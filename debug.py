...
def test_config_consistency():
    ...
    # FCD_DIM is FCD summary stats dim (not FC_DIM) — skip equality check
    ...
    # WC-EIB: STAGE1_PARAMS are global scalars P, Q, g_e, g_i, c_ei.
    assert all(isinstance(n, str) for n in cfg.STAGE1_PARAMS), (
        "STAGE1_PARAMS must be a list of parameter-name strings"
    )
    print(
    ...
    print(
        f"  STAGE1_PARAMS: {len(cfg.STAGE1_PARAMS)}개 global scalars "
        f"({', '.join(cfg.STAGE1_PARAMS)})"
    )
    print(
...
def test_parameter_scaler():
    ...
    print(f"  STAGE1_PARAMS: {len(cfg.STAGE1_PARAMS)}개 global scalars")
...
def test_stage2_param_scaler():
    """SKIP: Stage 2 removed from WC-EIB pipeline."""
    print("  SKIP: Stage 2 removed from WC-EIB pipeline")
    return "skip"
...
def test_fc_pca_scaler():
    """SKIP: PCA/MLP embedding removed from WC-EIB pipeline."""
    print("  SKIP: PCA/MLP embedding removed from WC-EIB pipeline")
    return "skip"


def test_feature_pipeline():
    """FeaturePipeline raw FC passthrough (no PCA)."""
    import config as cfg
    ...
    # USE_FCD defaults to False -> FC raw passthrough only.
    expected_dim = cfg.FC_DIM + (cfg.FCD_DIM if pipe.use_fcd else 0)
    assert x.shape == (n, expected_dim), (
        f"output {x.shape} != ({n}, {expected_dim})"
    )
    print(
        f"  pipeline: FC({cfg.FC_DIM}) raw passthrough "
        f"(USE_FCD={pipe.use_fcd}) -> {x.shape[1]}"
    )
...
def test_embedding_net():
    """SKIP: MLP FeatureEmbedding removed (RegionTransformer is Phase 3)."""
    print("  SKIP: PCA/MLP embedding removed from WC-EIB pipeline")
    return "skip"
...
def test_fcd_summary():
    """SKIP: FCD not used in WC-EIB pipeline (FC only)."""
    print("  SKIP: FCD not used in WC-EIB pipeline (FC only)")
    return "skip"
...
# ---------------------------------------------------------------------------
# WC-EIB pipeline: RegionTransformer, attention
# ---------------------------------------------------------------------------

def test_region_transformer():
    """RegionTransformerEmbedding forward pass (Phase 2 raw + Phase 4 masked)."""
    import config as cfg
    try:
        import torch
    except ImportError as e:
        print(f"  torch missing -> skip: {e}")
        return "skip"
    ...
    sc = np.random.rand(cfg.N_REGIONS, cfg.N_REGIONS).astype(np.float32)
    sc = (sc + sc.T) / 2

    # Phase 2: raw full upper-tri FC, no mask.
    net2 = make_embedding_net(
        "region_transformer",
        input_dim=cfg.FC_DIM,
        sc_matrix=sc,
        use_mask=False,
    )
    assert isinstance(net2, RegionTransformerEmbedding)
    out2 = net2(torch.randn(2, cfg.FC_DIM))
    assert out2.shape == (2, cfg.EMBED_DIM)
    print(f"  Phase2: (2,{cfg.FC_DIM}) → (2,{cfg.EMBED_DIM}) OK")

    # Phase 4: k selected upper-tri entries + mask channel (as run_phase3).
    k = cfg.SELECTION_K
    rr, cc = np.triu_indices(cfg.N_REGIONS, k=1)
    triu = (rr[:k], cc[:k])
    net4 = make_embedding_net(
        "region_transformer",
        input_dim=k,
        sc_matrix=sc,
        triu_indices=triu,
    ...
    out4 = net4(torch.zeros(2, k))
    assert out4.shape == (2, cfg.EMBED_DIM)
    assert net4.token_proj.in_features == 2 * cfg.N_REGIONS
    print(f"  Phase4: (2,{k})+mask → (2,{cfg.EMBED_DIM}) OK")
    print(f"  token_proj: {net4.token_proj.in_features}=2×{cfg.N_REGIONS}")


def test_attention_selector():
    """AttentionSelector: import + 필수 메서드 존재 확인."""
    import config as cfg
    import inspect
    from inference.attention_selection import AttentionSelector

    src = inspect.getsource(AttentionSelector)
    assert "get_attention_maps" in src
    assert "get_gradients" in src
    assert "get_importance" in src
    assert "select_top_k" in src
    print("  AttentionSelector: all methods present")
    print(f"  SELECTION_K={cfg.SELECTION_K}")


def test_feature_pipeline_passthrough():
    """FeaturePipeline: raw FC passthrough (no PCA)."""
    import config as cfg
    from inference.feature_pipeline import FeaturePipeline

    fc = np.random.randn(10, cfg.FC_DIM).astype(np.float32)
    pipe = FeaturePipeline()
    pipe.fit(fc, None, verbose=False)
    x = pipe.transform(fc, None)
    x2 = pipe.transform(fc, None)
    assert x.shape == (10, cfg.FC_DIM)
    assert x2.shape == (10, cfg.FC_DIM)
    print(f"  fit:       (10,{cfg.FC_DIM}) → (10,{cfg.FC_DIM}) OK")
    print(f"  transform: (10,{cfg.FC_DIM}) → (10,{cfg.FC_DIM}) OK")
    print("  PCA: None (raw FC passthrough)")
...
def test_mock_inference_flow():
    """SKIP: legacy mock flow (PCA + Stage 2) removed from WC-EIB pipeline."""
    print("  SKIP: PCA/Stage-2 mock flow removed from WC-EIB pipeline")
    return "skip"
...
def test_real_simulation():
    ...
    try:
        # WC-EIB: simulate_single uses fixed WC_FIXED params only
        params = {}
...
def run_basic_tests(runner):
    ...
    # WC-EIB pipeline structure
    runner.run("RegionTransformer", test_region_transformer)
    runner.run("AttentionSelector", test_attention_selector)
    runner.run("FeaturePipeline (pass)", test_feature_pipeline_passthrough)

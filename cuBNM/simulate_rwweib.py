"""Drop-in adapter: route VBI ``simulate_gpu_batch`` calls to cuBNM RWWEIB.

Same signature/return as ``cuBNM/simulate.py`` (the WCVBI adapter), but drives
the RWW-EIB-FFI model (``RWWEIBSimGroup`` via
``cuBNM.runner_rwweib.run_cubnm_rwweib_batch``). Selected by
``engine="rwweib"`` in ``inference/training_data.py``.
...
def simulate_gpu_batch(weights, theta_batch, param_names=None,
    ...
    """cuBNM RWWEIB implementation of the VBI ``simulate_gpu_batch`` contract.
    ...
    if not apply_bw:
        raise NotImplementedError(
            "cuBNM.simulate_rwweib.simulate_gpu_batch supports apply_bw=True "
    ...
    if use_delays and has_delays:
        ...
        if not _DELAY_WARNED:
            print("[cuBNM-RWWEIB] conduction delays ENABLED "
                  "(delay matrix fed as sc_dist, v=1.0).", flush=True)
            _DELAY_WARNED = True
    elif has_delays and not _DELAY_WARNED:
        print("[cuBNM-RWWEIB] WARNING: delays present but config.USE_DELAYS "
    ...
    if label is not None:
        ...
        print(f"[cuBNM-RWWEIB] {label}: running {len(theta_batch):,} sims "
...
def simulate_single(weights, params_dict, n_repeat=1, delays=None,
    ...
    """cuBNM RWWEIB drop-in for VBI ``simulator.simulate_single``."""
    import numpy as np
    import config

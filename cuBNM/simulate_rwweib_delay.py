"""Drop-in adapter: route VBI ``simulate_gpu_batch`` calls to cuBNM RWWEIB_DELAY.

Same signature/return as ``cuBNM/simulate.py`` (the WCVBI adapter), but drives
the RWW-EIB-FFI+DELAY model (``RWWEIBSimGroup`` via
``cuBNM.runner_rwweib_delay.run_cubnm_rwweib_delay_batch``). Selected by
``engine="rwweibdelay"`` in ``inference/training_data.py``.
...
def simulate_gpu_batch(weights, theta_batch, param_names=None,
    ...
    """cuBNM RWWEIB_DELAY implementation of the VBI ``simulate_gpu_batch`` contract.
    ...
    import numpy as np
    import config
    from cuBNM.runner_rwweib_delay import run_cubnm_rwweib_delay_batch

    if not apply_bw:
        raise NotImplementedError(
            "cuBNM.simulate_rwweib_delay.simulate_gpu_batch supports apply_bw=True "
    ...
    if use_delays and has_delays:
        ...
        if not _DELAY_WARNED:
            print("[cuBNM-RWWEIBDELAY] conduction delays ENABLED "
                  "(delay matrix fed as sc_dist, v=1.0).", flush=True)
            _DELAY_WARNED = True
    elif has_delays and not _DELAY_WARNED:
        print("[cuBNM-RWWEIBDELAY] WARNING: delays present but config.USE_DELAYS "
    ...
    if label is not None:
        ...
        print(f"[cuBNM-RWWEIBDELAY] {label}: running {len(theta_batch):,} sims "
              f"(of {_n:,}) on GPU ...", flush=True)

    bolds = run_cubnm_rwweib_delay_batch(
...
def simulate_single(weights, params_dict, n_repeat=1, delays=None,
    ...
    """cuBNM RWWEIB_DELAY drop-in for VBI ``simulator.simulate_single``."""
    import numpy as np
    import config
